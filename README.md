...
Google フォームと連携している既存のスプレッドシートにGASプロジェクトを作成します。

**手順:**

1. **温度・湿度データが入っているスプレッドシートを開く**
   - Google フォームの回答が記録されているスプレッドシート

2. **Apps Script エディタを開く**
   - メニューから「拡張機能」→「Apps Script」を選択
   - 新しいタブでGASエディタが開きます

3. **プロジェクト設定からスクリプトIDを取得**
   - 左側のメニューから「プロジェクトの設定」（歯車アイコン⚙️）をクリック
   - 「スクリプト ID」の欄に表示されているIDをコピー
   - 例: `1a2b3c4d5e6f7g8h9i0j1k2l3m4n5o6p7q8r9s0t`

4. **ローカルで .clasp.json を作成**

   コピーしたスクリプトIDを使って、プロジェクトのルートディレクトリに `.clasp.json` を作成します：
   ...
   `YOUR_SCRIPT_ID_HERE` を、先ほどコピーしたスクリプトIDに置き換えてください。
...
**注意**: `.clasp.json` は `.gitignore` に含まれているため、GitHubにコミットされません。
...
プッシュ後、GASエディタで `updateChart` 関数を実行してグラフを更新してください。
