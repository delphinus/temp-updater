...
- 最近2日間の温度・湿度データを自動抽出
- 見やすい折れ線グラフを自動生成
- 外気温（実測）を気象庁アメダスから取得して重ねて表示
- 今後 24 時間の外気温予報（Open-Meteo / 気象庁 JMA モデル）を別系列（別色）で表示
  - 予報は -24 時間ずらして「24 時間前〜現在」に重ね、同じ時刻帯で実測（今日）と予報（翌日同時刻）を比較できる
- 室内センサーのデータが途切れても、外気温（実測）・外気温予報・降水確率予報でグラフを現在まで継続
  - 最後のセンサー時刻から現在までの毎正時の行を補完し、室内温度・湿度の折れ線は欠測時刻で途切れ、外気温・予報は現在まで伸びる
  - 欠測中はグラフのサブタイトルに「※室内データは M/d HH:mm 以降欠測」と表示（データ欠損の Slack 通知は従来どおり送信）
- 最近2日間グラフの下に、降水量（実測）・降水確率（予報）の棒グラフを別途表示
  - 降水量（実測）は気象庁アメダスの 1 時間降水量、降水確率（予報）は Open-Meteo から取得（毎正時・1 時間刻み）
  - 降水確率（予報）は外気温予報と同じく今後 24 時間分を -24 時間ずらして「24 時間前〜現在」に重ねる
- 毎時自動更新（トリガー設定）
- TypeScriptで型安全に開発
