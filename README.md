...
スプレッドシート紐付き型のGASプロジェクトを作成します。

```bash
npx clasp create --type sheets --title "温度・湿度グラフ更新" --rootDir ./dist
```

このコマンドを実行すると、ブラウザが開いて権限を求められます。

**重要**: 権限の承認画面で、必ず以下のチェックボックスをオンにしてください：
- 「Google スプレッドシートのすべてのスプレッドシートの参照、編集、作成、削除」
- その他、必要な権限

その後、対象のスプレッドシート（温度・湿度データが入っているもの）を選択します。

**権限エラーが出た場合:**

`Insufficient Permission` エラーが出た場合は、権限のチェックボックスを見落としている可能性があります。以下を試してください：
...
または、スプレッドシートから直接作成する方法もあります：
1. スプレッドシートで「拡張機能」→「Apps Script」
2. 「プロジェクトの設定」からスクリプトIDを取得
3. `.clasp.json` を手動作成（詳細は後述）
